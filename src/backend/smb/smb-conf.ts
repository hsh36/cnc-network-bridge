...
export interface SmbShareConfig {
  ...
  /** Local cache directory, which is also the Samba export root. Must sit under /srv/smb-bridge. */
...
export interface SmbConfInput {
  ...
  readonly machineInterface: string;
  /**
...
# Generated by SMB Bridge. Do not edit by hand.
...
export function renderSmbConf(input: SmbConfInput): string {
  ...
  const view = {
    ...
    netbiosName: sanitiseValue(input.netbiosName ?? 'SMB-BRIDGE', 'netbiosName'),
    serverString: sanitiseValue(input.serverString ?? 'SMB Bridge', 'serverString'),
    maxProtocol: input.maxProtocol ?? 'SMB3',
    ntlmAuth: input.ntlmAuth ?? true,
    machineInterface: sanitiseValue(input.machineInterface, 'machineInterface'),
...
export function assertSafeConfig(content: string, options: SafetyOptions): void {
  ...
  if (!interfaces.includes(options.machineInterface)) {
    failures.push(
      `global/interfaces does not include the TNC interface "${options.machineInterface}"`,
    );
  }
  ...
  const extra = interfaces.filter(
    (entry) => entry !== options.machineInterface && entry !== 'lo' && entry !== '127.0.0.1',
  );
  if (extra.length > 0) {
...
export function buildSmbConf(input: SmbConfInput): string {
  ...
  assertSafeConfig(content, {
    machineInterface: input.machineInterface,
